...
for(runID in runID_ls){
  ...
  #sim_yrs <- seq(2015, 2016, 1)   #movie made for all these years (will usually be identical to yrs above)
  fig_yrs <- c(2001, 2005, 2010, 2015) #figures output for only these years 
  #fig_yrs <- seq(2015, 2016, 1)#figures output for only these years 
  ...
  getFRs <- function(data)
  {
    ...
    LU %>%
      ...
      dplyr::summarise(FR1 = round(sum(LandUse == 'FR1') / n(), 3),
                       ...
                       FR1236 = round(sum(LandUse == 'FR1' | LandUse == 'FR2' | LandUse == 'FR3' | LandUse == 'FR6') / n(), 3),
  ...
  cDat <- cDat %>%
    mutate(ModMode = factor(ModMode, c(1,3,4,5))) %>%
    mutate(ObsMode = factor(ObsMode, c(1,3,4,5)))
  
  
  #could create a table subset here of munis that do not match?
  
  summary(cDat)
  LCnames <- c("Nature", "Arable", "Other", "Pasture")  #used to label error matrix in loop below
  ...
  ddf <- data_frame(
    Year = rep(calib_yrs,length.out=4),
    LC = c(1,3,4,5) 
  )
  ...
  mods <- mods %>%
    ...
    mutate(LC = factor(LC, labels = c("Nature", "Arable", "Other","Pasture"))) 
  
  print(mods)
  grid.arrange(tableGrob(mods), top="Model Parameters")
  
  #create colours for plot
  myCols <- c("forestgreen","wheat3","black","orange2")
  names(myCols) <- c('Nature',"Arable","Other","Pasture")
  ...
  cDat_long <- cDat_long %>%
    mutate(LC = 
      ...
      if_else(LC == "Agri.Mod" | LC == "Agri.Obs", "Arable",
  ...
  c <- cDat_long %>% 
    mutate(ha = cells * 25) %>%
    ggplot(aes(x = Year, y = ha, color = LC, linetype = Source)) + 
    geom_line(size = 1) +
    scale_colour_manual(name = "LUC",values = myCols) +
    scale_y_continuous(name = "Area (sq km)", labels = scales::comma) +
  ...
  lc_pal <- c("forestgreen", "wheat2", "black", "orange2")
  lc_labs <- c("Nature", "Agriculture", "Other", "Pasture")
  ...
  for(yr in sim_yrs){
    ...
    if(yr %in% fig_yrs) {
      ...
      #for diverging pallete: https://stackoverflow.com/questions/37482977/what-is-a-good-palette-for-divergent-colors-in-r-or-can-viridis-and-magma-b
  ...
  if(length(states) == 0)
  {
    ...
    odata <- read_csv(data_dir,"Production_Export_allBrazil.csv")
